...
class UserVideo extends Model
{
    ...
    public function getSkorById($user_id, $video_id)
    {
        $builder = $this->db->table('user_video');
        $builder->select('score');
        $builder->where(['user_id' => $user_id, 'video_id' => $video_id]);
        $query = $builder->get();

        return $query->getRow();
    }

    public function getMemberByCourseId($userId, $courseId)
    {
        return $this->builder()->select('user_video.*,video.title')->join('video', 'user_video.video_id = video.video_id')->where('user_id', $userId)->where('course_id', $courseId)->get()->getResultArray();
    }
}
